...
if (appRoot) {
  appRoot.innerHTML = renderRuntimeSwitchNotes(createDefaultModel());
}

function createDefaultModel() {
  return {
    runtimes: [
      { name: "Node.js", port: 4171, runTarget: "node-server", state: "ready" },
      { name: "Bun", port: 4172, runTarget: "bun-server", state: "optional" },
      { name: "Deno", port: 4173, runTarget: "deno-server", state: "optional" },
    ],
    notes: [
      { id: "note-1", text: "Switch the workspace runtime profile with one manifest line." },
      { id: "note-2", text: "RunTargets stay explicit about node, bun, and deno." },
    ],
  };
}

function renderRuntimeSwitchNotes(model) {
