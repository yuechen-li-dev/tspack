...
Outcome B: the sample is worth keeping as a future release smoke, and M43b removed the update/store recursion blocker, but the dogfooding run still has other concrete workflow blockers.
...
Before release, the remaining highest-value fixes are URL inspect requiring VS Code and the bridge/build output mismatch. After those are fixed, this example should become a useful end-to-end release smoke.
