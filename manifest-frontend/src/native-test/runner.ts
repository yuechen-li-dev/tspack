...
type ProjectContext = {
  ...
  writeText: (relativePath: string, text: string, reason: string) => Promise<void>;
  writeJson: (relativePath: string, value: unknown, reason: string) => Promise<void>;
  writeBytes: (relativePath: string, bytes: Uint8Array | Buffer, reason: string) => Promise<void>;
};

const SKIP_FIXTURE_DIRS = new Set(['node_modules', '.git', '.tspack', 'tspack-artifacts', 'dist-packages']);

export async function runSuite(root: RuntimeNode, options: RunSuiteOptions = {}): Promise<TestResult[]> {
  if (!root || root.__tag !== 'Suite') throw new Error('suite root must be Suite node');
...
async function runSingle(id: string, name: string, declarations: DiscoveredArtifact[], projectDecl: DiscoveredProjectFixture | undefined, artifactRoot: string, timeoutSeconds: number, fn: (ctx: TestContext) => unknown): Promise<TestResult> {
  ...
  try {
    ...
    return { id, name, status: 'passed', durationMs: performance.now() - started, artifacts: state.results, project: projectResult };
  } catch (error) {
    if (isSkipSignal(error)) {
      clearPendingExpectations();
      if (sandboxRoot) await fs.rm(sandboxRoot, { recursive: true, force: true });
      return { id, name, status: 'skipped', durationMs: performance.now() - started, skipReason: error.skipReason, artifacts: state.results, project: projectDecl ? { sourcePath: projectDecl.from, name: projectDecl.name, kept: false } : undefined };
    }
    if (projectDecl && sandboxRoot) {
...
async function createProjectContext(id: string, projectDecl: DiscoveredProjectFixture): Promise<ProjectContext> {
  const rootPath = await fs.mkdtemp(path.join(os.tmpdir(), `tspack-project-${sanitizeId(id)}-`));
  if (projectDecl.from) await copyFixtureDirectory(path.resolve(projectDecl.from), rootPath);
  const resolveSafe = (rel: string): string => {
    if (!rel || !rel.trim() || rel.startsWith('/') || rel.includes('..') || rel.includes('\\')) throw withCode('TSPACK_PROJECT_INVALID_PATH', `invalid project path: ${rel}`);
    const resolved = path.resolve(rootPath, rel);
    if (!resolved.startsWith(`${rootPath}${path.sep}`) && resolved !== rootPath) throw withCode('TSPACK_PROJECT_PATH_ESCAPE', `project path escapes sandbox: ${rel}`);
    return resolved;
  };
  const requireReason = (reason: string): void => {
    if (!reason || !reason.trim()) throw withCode('TSPACK_PROJECT_WRITE_REASON_REQUIRED', 'project write reason is required');
  };
  return {
    rootPath,
    ...
    readText: async (relativePath) => {
      try { return await fs.readFile(resolveSafe(relativePath), 'utf8'); } catch (e) { throw withCode('TSPACK_PROJECT_READ_FAILED', (e as Error).message); }
    },
    readJson: async (relativePath) => JSON.parse(await fs.readFile(resolveSafe(relativePath), 'utf8')),
    writeText: async (relativePath, text, reason) => { requireReason(reason); await writeProjectFile(resolveSafe(relativePath), Buffer.from(text, 'utf8')); },
    writeJson: async (relativePath, value, reason) => { requireReason(reason); await writeProjectFile(resolveSafe(relativePath), Buffer.from(`${JSON.stringify(value, null, 2)}\n`, 'utf8')); },
    writeBytes: async (relativePath, bytes, reason) => { requireReason(reason); await writeProjectFile(resolveSafe(relativePath), Buffer.from(bytes)); },
  };
}
...
function isNode(value: unknown): value is RuntimeNode { if (!value || typeof value !== 'object') return false; const candidate = value as RuntimeNode; return typeof candidate.__tag === 'string' && !!candidate.props && Array.isArray(candidate.children); }
