...
type PendingToken = { finalize: (reason: string) => void };

const pendingStorage = new AsyncLocalStorage<Set<PendingToken>>();
const fallbackPending = new Set<PendingToken>();

export function setPendingExpectationScope(
  next: Set<PendingToken> | undefined,
): void {
  pendingStorage.enterWith(next ?? fallbackPending);
}

function currentPending(): Set<PendingToken> {
  return pendingStorage.getStore() ?? fallbackPending;
}
...
function extractDiagnostics(subject: unknown): DiagnosticLike[] {
  ...
  if (
    ...
    Array.isArray((subject as { diagnostics?: unknown }).diagnostics)
  ) {
...
function buildPending(
  ...
  const token = {
    finalize(reason: string): void {
      ...
      const matched = useDeepEqual
        ? isDeepStrictEqual(actual, expected)
        : Object.is(actual, expected);
      const ok = negate ? !matched : matched;
      if (!ok)
        throw expectationFailure(
          "TSPACK_ASSERT_FAILURE",
          `${negate ? "not." : ""}${useDeepEqual ? "toEqual" : "toBe"}`,
          reason,
          expected,
          actual,
        );
  ...
  currentPending().add(token);
  return {
    because(reason: string): void {
      markExpectationActivity();
      currentPending().delete(token);
      token.finalize(reason);
    },
  };
}

function buildErrorExpectation(
  subject: unknown,
  expectedCode: string,
): PendingExpectation {
  const token = {
    finalize(reason: string): void {
      ...
      const codes = diagnostics.map((entry) =>
        String((entry as { code?: string }).code ?? ""),
      );
      if (!codes.includes(expectedCode)) {
        throw expectationFailure(
          "TSPACK_EXPECT_ERROR_NOT_FOUND",
          "error",
          reason,
          expectedCode,
          codes,
  ...
  const token = {
    finalize(reason: string): void {
      ...
      if (errorCodes.length > 0) {
        throw expectationFailure(
          "TSPACK_EXPECT_UNEXPECTED_ERRORS",
          "noErrors",
          reason,
          [],
          errorCodes,
...
function baseExpect(actual: unknown) {
  const mk = (negate: boolean) => ({
    toBe: (expected: unknown) => buildPending(actual, expected, false, negate),
    toEqual: (expected: unknown) =>
      buildPending(actual, expected, true, negate),
  });
...
expect.error = (subject: unknown, code: string): PendingExpectation =>
  buildErrorExpectation(subject, code);
expect.noErrors = (subject: unknown): PendingExpectation =>
  buildNoErrorsExpectation(subject);
expect.noError = (subject: unknown): PendingExpectation =>
  buildNoErrorsExpectation(subject);
expect.snapshotText = (value: unknown, name: string): PendingExpectation =>
  buildSnapshotExpectation(value, name, "text");
expect.snapshotJson = (value: unknown, name: string): PendingExpectation =>
  buildSnapshotExpectation(value, name, "json");
