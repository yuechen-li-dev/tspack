...
import {
  ...
  Suite,
  Theory,
} from "../../src/native-test/index";

describe("native runner theory structure", () => {
  it("runs theory callbacks before, after, and between cases with case-order IDs", async () => {
    const root = Suite(
      { name: "order" },
      Theory(
        { name: "after" },
  ...
  it("fails invalid theory structures deterministically at runtime", async () => {
    const root = Suite(
      { name: "bad" },
      Theory({ name: "zero" }, () => {
        assert.true(true, "should not run zero-case theory");
      }),
      Theory({ name: "missing" }, Case({ value: 1 })),
      Theory(
        { name: "duplicate" },
        Case({ value: 1 }),
        () => {
          assert.true(true, "first duplicate body");
        },
        () => {
          assert.true(true, "second duplicate body");
        },
      ),
    );
    ...
    const codes = results.map(
      (result) => (result.error as { code?: string } | undefined)?.code,
    );
  ...
  it("keeps no-assertion and skip behavior per callback-before-cases theory case", async () => {
    const root = Suite(
      { name: "meaningful" },
      Theory(
        { name: "no assertion" },
      ...
      Theory(
        ...
        ({ value }) => {
          ...
          assert.true(true, "non-skipped case asserts");
        },
        Case({ value: 1 }),
        Case({ value: 2 }),
      ),
    ...
    expect(results[2].skipReason).toBe("skip case two");
  });
});
...
describe("native runner artifacts", () => {
  it("writes text/json/bytes artifacts and records metadata", async () => {
    const artifactRoot = fs.mkdtempSync(
      path.join(os.tmpdir(), "native-artifacts-"),
    );
    const root = Suite(
      { name: "math" },
      Fact(
  ...
  it("fails on unknown, duplicate, missing reason, required not written; optional and skip behavior", async () => {
    ...
    const root = Suite(
      ...
      Fact(
        { name: "unknown" },
        Artifact({ name: "x", path: "x.txt" }),
        async ({ artifact }) => artifact.writeText("y", "v", "r"),
      ),
      Fact(
        { name: "dup" },
        Artifact({ name: "x", path: "x.txt" }),
        async ({ artifact }) => {
          await artifact.writeText("x", "a", "r");
          await artifact.writeText("x", "b", "r");
        },
      ),
      Fact(
        { name: "reason" },
        Artifact({ name: "x", path: "x.txt" }),
        async ({ artifact }) => artifact.writeText("x", "a", ""),
      ),
      Fact(
        { name: "required" },
        Artifact({ name: "x", path: "x.txt" }),
      ...
      Fact(
        { name: "optional" },
        Artifact({ name: "x", path: "x.txt", optional: true }),
        () => {
          assert.true(true, "optional still asserts");
        },
      ),
      Fact({ name: "skip" }, Artifact({ name: "x", path: "x.txt" }), () => {
        skip("later");
      }),
    );
    const results = await runSuite(root, { artifactRoot });
    expect((results[0].error as { code?: string }).code).toBe(
      "TSPACK_ARTIFACT_UNKNOWN",
    );
    expect((results[1].error as { code?: string }).code).toBe(
      "TSPACK_ARTIFACT_ALREADY_WRITTEN",
    );
    expect((results[2].error as { code?: string }).code).toBe(
      "TSPACK_ARTIFACT_REASON_REQUIRED",
    );
    expect((results[3].error as { code?: string }).code).toBe(
  ...
  it("writes theory case artifacts to separate directories", async () => {
    ...
    const root = Suite(
      ...
      Theory(
        { name: "cases" },
        Artifact({ name: "r", path: "report.txt" }),
        Case({ n: 1 }),
        Case({ n: 2 }),
        async ({ n }, { artifact }) => {
          await artifact.writeText("r", String(n), "record");
          assert.true(true, "case asserted");
        },
      ),
    );
    const results = await runSuite(root, { artifactRoot });
    ...
    expect(
      fs.existsSync(path.join(artifactRoot, "t__cases__0", "report.txt")),
    ).toBe(true);
    expect(
      fs.existsSync(path.join(artifactRoot, "t__cases__1", "report.txt")),
    ).toBe(true);
  });
});
...
describe("native runner project fixtures", () => {
  it("creates sandbox, supports reads and writes, and cleans up on pass", async () => {
    const fixtureRoot = fs.mkdtempSync(
      path.join(os.tmpdir(), "native-project-fixture-"),
    );
    ...
    const root = Suite(
      ...
      Fact(
        ...
        async ({ project }) => {
          ...
          const parsed = await project!.readJson<{ x: number }>(
            "manifest.json",
          );
          assert.equal(parsed.x, 1, "reads copied fixture json");
          await project!.writeText(
            "generated.txt",
            "hello\n",
...
it("enforces project path/write/read safety and keepOnFailure semantics", async () => {
  ...
  const root = Suite(
    { name: "px" },
    Fact({ name: "empty" }, Project({}), async ({ project }) => {
      assert.true(!!project?.rootPath, "has root");
    }),
    Fact(
      { name: "path" },
      Project({ from: fixtureRoot }),
    ...
    Fact(
      ...
      Project({ from: fixtureRoot }),
      async ({ project }) => {
        await project!.writeText("x.txt", "x", "");
      },
    ),
    Fact(
      { name: "read missing" },
      Project({ from: fixtureRoot }),
      async ({ project }) => {
        await project!.readText("missing.txt");
      },
    ),
    Fact(
      { name: "write/json/bytes/read" },
      Project({ from: fixtureRoot }),
      async ({ project }) => {
        await project!.writeText("sub/t.txt", "hello", "write text");
        await project!.writeJson("sub/j.json", { z: 1, a: 2 }, "write json");
        await project!.writeBytes(
          "sub/b.bin",
          new Uint8Array([1, 2, 3]),
          "write bytes",
        );
        assert.equal(
          await project!.readText("sub/t.txt"),
          "hello",
          "read text",
        );
        const json = await project!.readJson<{ z: number; a: number }>(
          "sub/j.json",
        );
        assert.equal(json.a, 2, "read json");
        assert.true(fs.existsSync(project!.path("sub/b.bin")), "bytes exists");
        assert.true(
          !fs.existsSync(path.join(project!.rootPath, "node_modules")),
    ...
    Theory(
      ...
      async ({ n }, { project }) => {
        ...
        assert.true(
          fs.existsSync(project!.path(`case-${n}.txt`)),
          "file in case sandbox",
        );
      },
    ),
    Fact(
      { name: "artifact and project" },
      Project({}),
      Artifact({ name: "a", path: "a.txt" }),
      async ({ artifact, project }) => {
    ...
    Fact(
      { name: "keep false fail" },
      Project({ keepOnFailure: false }),
      async () => {
    ...
    Fact(
      { name: "keep true fail" },
      Project({ keepOnFailure: true }),
      async () => {
        assert.equal(1, 2, "fail keep");
      },
    ),
    Fact(
      { name: "keep true pass" },
      Project({ keepOnFailure: true }),
      async ({ project }) => {
        await project!.writeText("ok.txt", "ok", "pass");
        assert.true(true, "pass has assertion");
      },
    ),
    Fact({ name: "skip cleanup" }, Project({}), async () => {
      skip("later");
    }),
  );
  ...
  expect((byId.get("px/write reason")?.error as { code?: string })?.code).toBe(
    "TSPACK_PROJECT_WRITE_REASON_REQUIRED",
  );
  expect((byId.get("px/read missing")?.error as { code?: string })?.code).toBe(
    "TSPACK_PROJECT_READ_FAILED",
  );
...
describe("native runner command helpers", () => {
  it("provides command context only for project tests and captures evidence", async () => {
    const artifactRoot = fs.mkdtempSync(
      path.join(os.tmpdir(), "native-command-artifacts-"),
    );
    const root = Suite(
      { name: "cmd" },
      ...
      Fact(
        ...
        async ({ command, project }) => {
          ...
          assert.true(
            result.stdout.includes(project!.rootPath),
            "command should run in project cwd",
          );
        },
      ),
    );
    ...
    const payload = JSON.parse(
      fs.readFileSync(path.join(commandDir, "0.command.json"), "utf8"),
    );
  ...
  it("supports invalid args/reason, timeout, nonzero, and invalid cwd", async () => {
    const root = Suite(
      ...
      Fact({ name: "invalid-cwd" }, Project({}), async ({ command }) => {
        await command!.run(["node", "-e", "console.log(1)"], "bad cwd", {
          cwd: "../escape",
        });
      }),
    ...
    expect(results[1].status).toBe("passed");
    expect((results[2].error as { code?: string }).code).toBe(
      "TSPACK_COMMAND_INVALID_CWD",
    );
    expect((results[3].error as { code?: string }).code).toBe(
      "TSPACK_COMMAND_REASON_REQUIRED",
    );
  });
    ...
    try {
      const root = Suite(
        ...
        Fact({ name: "tspack" }, Project({}), async ({ command, project }) => {
          fs.mkdirSync(path.join(project!.rootPath, "sub"), {
            recursive: true,
          });
          const result = await command!.tspack(
            ["alpha", "beta"],
            "call tspack",
            { cwd: "sub", env: { MARKER: "yes" } },
          );
