...
func packageJSONDependencyEntries(pkg packageJSON) []dependencyEntry {
	...
	for _, section := range sections {
		for name, versionRange := range section.deps {
			entries = append(entries, dependencyEntry{name: name, section: section.name, versionRange: versionRange})
		}
	}
...
func findingSortKey(finding AnnotationCheckFinding) string {
	encoded, err := json.Marshal([]string{finding.PackageRoot, finding.Severity, finding.Code, finding.DependencyName})
	if err != nil {
