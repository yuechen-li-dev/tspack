...
var entries = []DiagnosticHelp{
	...
	{Code: "TSPACK_BOUNDARY_TOOL_RUNTIME_IMPORT", Title: "Tool dependency imported in runtime source", Summary: "Runtime code imported a tool-only dependency.", Why: "Tool dependencies are not part of runtime dependency closure and are not guaranteed at execution time.", CommonCauses: []string{"Importing build or test tooling from application/library runtime modules.", "Confusing tool and runtime dependency kinds in manifest declarations."}, Fixes: []string{"Move the import into build/test/tool scripts.", "Replace with a runtime dependency when the import is truly required at runtime.", "Change dependency kind deliberately after validating runtime impact."}},
	{Code: "TSPACK_BOUNDARY_EXPLICIT_DENY", Title: "Explicit runtime boundary deny rule", Summary: "A runtime import matched a deny boundary policy.", Why: "Boundary rules keep package contracts enforceable and prevent accidental coupling.", Fixes: []string{"Move the import behind an allowed package boundary.", "Refactor shared logic into a package allowed by policy.", "Update boundary policy deliberately if architecture changed."}},
