...
// RuntimeSource returns a copy of the embedded browser runtime used during
// materialization.
func RuntimeSource() []byte {
	return append([]byte(nil), browserRuntimeSource...)
}
...
type Materialization struct {
	...
	AttachmentPlans *AttachmentPlans  `json:"attachmentPlans,omitempty"`
	ComponentFrames *ComponentFrames  `json:"componentFrames,omitempty"`
}

// AttachmentPlans is transport metadata only. TSPack validates and
// delivers Copeland's plan; it never infers adapters, capabilities, or hosts.
type AttachmentPlans struct {
	SchemaVersion int    `json:"schemaVersion"`
...
// ComponentFrames identifies compiler-emitted transition code. The
// module is materialized unchanged; TSPack does not parse or infer it.
type ComponentFrames struct {
	SchemaVersion int    `json:"schemaVersion"`
	Path          string `json:"path"`
	SHA256        string `json:"sha256"`
}
...
func MaterializeGraph(outputDirectory string, contracts []NpmContract) (Materialization, error) {
	result := Materialization{SchemaVersion: 1, Imports: []Import{}, Packages: []PackageArtifact{}}
	for _, contract := range contracts {
		if !contract.Materialized {
			return Materialization{}, fmt.Errorf("browser package %q was resolved but is not materialized", contract.PackageName)
		}

		sourceDirectory := contract.MaterializationPath
		entry, err := SelectPackageEntry(sourceDirectory, PackageSubpath(contract.PackageName))
		if err != nil {
			return Materialization{}, fmt.Errorf("select browser entry for %q: %w", contract.PackageName, err)
		}
		...
		if IsCommonJSModule(sourceDirectory, entry) || shouldBundleBrowserPackage(contract.PackageName) {
			...
			if err != nil {
				return Materialization{}, fmt.Errorf("transform browser package %q: %w", contract.PackageName, err)
			}
		} else if err := copyBrowserPackage(sourceDirectory, packageDirectory); err != nil {
			return Materialization{}, fmt.Errorf("materialize browser package %q: %w", contract.PackageName, err)
		}

		outputPath := filepath.Join(outputDirectory, filepath.FromSlash(output))
		hash, err := browserArtifactHash(outputPath)
		if err != nil {
			return Materialization{}, fmt.Errorf("hash browser package %q: %w", contract.PackageName, err)
		}
		entryPath := "./" + output
		...
		result.Packages = append(result.Packages, PackageArtifact{
			Specifier:   contract.PackageName,
...
func validateComponentFrameArtifact(path string) (*ComponentFrames, error) {
...
func validateAttachmentPlanArtifact(path string) (*AttachmentPlans, error) {
	contents, err := os.ReadFile(path)
	if errors.Is(err, os.ErrNotExist) {
...
func SelectPackageEntry(directory string, subpath string) (string, error) {
...
func transformBrowserPackage(sourceDirectory string, entry string, outputPath string, specifier string) (string, error) {
	...
	transformerIdentity := TransformerIdentity()
...
func TransformerIdentity() string {
	esbuildPath, err := bundledEsbuildPath()
	if err != nil {
...
func WriteHost(packageRoot string, outputDirectory string, entryOutputPath string, materialization Materialization) error {
...
func hasBrowserImport(materialization Materialization, specifier string) bool {
	return slices.ContainsFunc(materialization.Imports, func(item Import) bool {
		return item.Specifier == specifier
	})
}

func MaterializationFingerprint(materialization Materialization) string {
	hash := sha256.New()
	for _, item := range materialization.Imports {
