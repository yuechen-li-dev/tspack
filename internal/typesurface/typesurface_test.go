...
func has(diags []string, x string) bool { for _, d := range diags { if d==x { return true } }; return false }
...
func TestLeakPathAndDeterminism(t *testing.T) {
	...
	for _, d := range a { if d.Code=="TSPACK_TYPE_OPTIONAL_PEER_LEAK" { p:=strings.Join(PathFromDetails(d),"|"); if !strings.Contains(p,"dist/index.d.ts") || !strings.Contains(p,"dist/text/index.d.ts") || !strings.Contains(p,"dist/text/vue/index.d.ts") || !strings.Contains(p,"vue") { t.Fatal(p) }; return } }
