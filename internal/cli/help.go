...
func printCommandHelp(command string) {
	pages := map[string][]string{
		"add":      {"tspack add <package>", "Authors an explicit dependency declaration in the selected native package and updates ts-lock.toml.", "Examples:", "  tspack add lodash", "  tspack add zod@^4", "  tspack add lodash --package @scope/app --dry-run", "Behavior:", "  npm is the default source. An unqualified add chooses the newest stable release and writes a caret constraint.", "  Explicit constraints are preserved exactly. Repeating an unqualified add of the same editable declaration is a no-op.", "  An explicit add shadows generated or concept-owned declarations without deleting their provenance.", "  Optional semantics are orthogonal to TSPack's normal dependency kind.", "  Multiple native packages require --package. Incremental package.json projects remain npm-authoritative.", "Important flags:", "  --root <path>      Project root", "  --package <name>   Select the native package to edit", "  --optional         Mark the dependency optional", "  --dry-run          Plan the semantic/source edit without writing", "  --json             Emit machine-readable output", "Related:", "  update          Resolve existing authoring intent", "  npm             Mutate package.json-native projects"},
		"remove":   {"tspack remove <package>", "Removes one owned, editable dependency declaration and updates ts-lock.toml.", "Examples:", "  tspack remove lodash", "  tspack remove zod --package @scope/app", "  tspack remove lodash --dry-run --json", "Behavior:", "  Removal targets authoring intent, not an npm version query or forced graph deletion.", "  A lower-precedence concept or template declaration may become effective again and is never rewritten.", "  If the package remains transitively resolved or is required by another workspace, output keeps that distinct from this package's direct declaration.", "  Repeated removal and derived-only matches are no-op operations.", "  Multiple native packages require --package. Incremental package.json projects remain npm-authoritative.", "Important flags:", "  --root <path>      Project root", "  --package <name>   Select the native package to edit", "  --optional         Select an optional declaration when needed", "  --dry-run          Plan the semantic/source edit without writing", "  --json             Emit machine-readable output", "Related:", "  add             Author an explicit dependency", "  update          Resolve existing authoring intent", "  npm             Mutate package.json-native projects"},
