...
func printDefaultHelp() {
	...
	fmt.Println("  add        Author a dependency and update ts-lock.toml")
	fmt.Println("  remove     Remove an editable dependency declaration and update ts-lock.toml")
	...
	fmt.Println("  tspack audit [--audit-level any|low|moderate|high|critical] [--json]")
	fmt.Println("  tspack inspect targets [--kind build|test] [--json]")
...
func printWorkflowHelp() {
	...
	fmt.Println("  tspack workflow run CI [--jobs 4] [--json]")
	...
	fmt.Println("and finite ForEach sources lower to explicit ordered cursors and aggregates. ParallelForEach is bounded;")
	fmt.Println("Transfer moves artifacts explicitly, and When branches on typed facts. Process uses argv, while")
...
func printHelpTopic(topic string) bool {
	...
	case "all", "legacy", "flags":
		printLegacyHelp()
	case "init", "add", "remove", "update", "sync", "build", "check", "run", "audit", "npm", "why", "outdated", "pack", "doctor", "test", "inspect", "adopt", "migrate", "format", "how":
...
func printCommandHelp(command string) {
	pages := map[string][]string{
		"add":      {"tspack add <package>", "Authors an explicit dependency declaration in the selected native package and updates ts-lock.toml.", "Examples:", "  tspack add lodash", "  tspack add zod@^4", "  tspack add @std/path --source jsr", "  tspack add foo --optional", "  tspack add foo --source npm", "  tspack add foo --package packages/lib", "  tspack add foo --dry-run --json", "Behavior:", "  npm is the default source. --source npm or --source jsr selects one registry explicitly; TSPack never searches other registries automatically.", "  An unqualified add chooses the newest stable release and writes a caret constraint. Explicit constraints are preserved exactly.", "  Repeating an equivalent editable declaration is a zero-registry, byte-for-byte no-op; an explicit changed constraint replaces it.", "  An explicit add shadows generated or concept-owned declarations without deleting their provenance.", "  Optional semantics are orthogonal to TSPack's normal dependency kind.", "  Package targets accept stable package names or exact workspace-relative roots. Invocation inside one package is inferred when unambiguous.", "  --dev is intentionally unavailable because TSPack's test kind is reserved. --tool is unavailable until dependency and <Tools> selection can be projected atomically.", "  Incremental package.json projects remain dependency-authoritative.", "Important flags:", "  --root <path>         Project root", "  --package <name|path> Select the native package to edit", "  --source npm|jsr      Select the registry source explicitly", "  --kind dep|peer       Select TSPack dependency semantics", "  --optional            Mark the dependency optional", "  --dry-run             Plan the semantic/source edit without writing", "  --json                Emit machine-readable output", "Related:", "  remove          Remove editable authoring intent", "  update          Resolve existing authoring intent", "  npm             Mutate package.json-native projects"},
		"remove":   {"tspack remove <package>", "Removes one owned, editable dependency declaration and updates ts-lock.toml.", "Examples:", "  tspack remove lodash", "  tspack remove @std/path --source jsr", "  tspack remove foo --package packages/lib", "  tspack remove foo --kind peer", "  tspack remove foo --optional", "  tspack remove lodash --dry-run --json", "Behavior:", "  Removal targets source-qualified authoring intent, not an npm version query or forced graph deletion.", "  --kind, --optional, and --source narrow ambiguous declarations consistently with add semantics.", "  A lower-precedence concept or template declaration may become effective again and is never rewritten.", "  If the package remains transitively resolved or is required by another workspace, output keeps that distinct from this package's direct declaration.", "  Repeated removal and derived-only matches are zero-registry, byte-for-byte no-op operations.", "  Package targets accept stable package names or exact workspace-relative roots. Invocation inside one package is inferred when unambiguous.", "  Incremental package.json projects remain dependency-authoritative.", "Important flags:", "  --root <path>             Project root", "  --package <name|path>     Select the native package to edit", "  --source npm|jsr          Select the registry source explicitly", "  --kind dep|peer|tool|test Narrow by TSPack dependency kind", "  --dev                     Alias for --kind test when removing", "  --tool                    Alias for --kind tool when removing", "  --optional                Select an optional declaration when needed", "  --dry-run                 Plan the semantic/source edit without writing", "  --json                    Emit machine-readable output", "Related:", "  add             Author an explicit dependency", "  update          Resolve existing authoring intent", "  npm             Mutate package.json-native projects"},
		...
		"audit":    {"tspack audit", "Checks exact npm package versions in ts-lock.toml against OSV.dev.", "Use in local review and CI after tspack update.", "Examples:", "  tspack audit", "  tspack audit --audit-level high", "  tspack audit --json", "Behavior:", "  Audit is read-only and queries OSV.dev using locked package name/version pairs.", "  Any finding fails by default; --audit-level raises the failure threshold.", "  Advisory-service failures are errors and are never reported as a clean scan.", "Important flags:", "  --root <path>          Project root", "  --audit-level <level>  any, low, moderate, high, or critical", "  --json                 Emit machine-readable output", "Related:", "  why              Show why a vulnerable package is present", "  update           Refresh selected dependency state", "  doctor security  Inspect lifecycle capability posture"},
		...
		"inspect":  {"tspack inspect", "Lists declared targets or locked package realizations, or inspects browser/app/runtime targets.", "Use discovery to find semantic identities without reading manifest or lock source.", "Usage:", "  tspack inspect targets [--kind build|test] [--json]", "  tspack inspect packages [--json]", "  tspack inspect <url> [experimental] [--run target] [--browser backend]", "Examples:", "  tspack inspect targets", "  tspack inspect packages --json", "  tspack inspect https://example.test", "  tspack inspect --run dev --bundle", "Important flags:", "  --kind <build|test>   Filter target discovery", "  --root <path>         Select the project root", "  --json                Emit deterministic machine-readable output", "  --run <target>        Start a RunTarget before browser inspection", "  --browser <backend>   Select browser backend", "  --selector <css>      Inspect a selector", "  --bundle              Emit deterministic UI context JSON", "  --bundle-output <file> Atomically replace a bundle file", "  --watch               Reinspect after relevant project changes", "  --watch-debounce <ms> Coalesce filesystem events (25-5000ms)", "  --verbose             Show bounded inspect/watch details", "Related:", "  build           Build a discovered target", "  test            Run a discovered test target", "  run             Start targets", "  doctor inspect  Diagnose inspect setup"},
