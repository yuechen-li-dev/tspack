...
func TestMigrateWriteForceAndAllOrNothing(t *testing.T) {
	...
	assertFileContains(t, reportPath, "Promote the reviewed draft to the root `manifest.tsx`")
	if content, err := os.ReadFile(reportPath); err != nil {
		t.Fatal(err)
	} else if strings.Contains(string(content), "--manifest manifest.migrated.tsx") {
		t.Fatalf("report suggested lifecycle commands that reject the draft filename:\n%s", content)
	}
}
...
func TestMigrateCheckDryRunSuccessWritesNothing(t *testing.T) {
  ...
  "scripts":{"dev":"vite","postinstall":"node postinstall.js"}
	...
	cmd := migrateCommandWithValidationFrontend(t, "migrate", "--root", root, "--check")
	output, err := cmd.CombinedOutput()
	if err != nil {
...
func TestMigrateCheckWriteSuccessReportValidation(t *testing.T) {
	...
	cmd := migrateCommandWithValidationFrontend(t, "migrate", "--root", root, "--write", "--check")
	output, err := cmd.CombinedOutput()
	if err != nil {
...
func TestMigratePhase8aScopedFixtureCheckRerun(t *testing.T) {
  ...
  "files": ["dist", "README.md", "LICENSE"]
	...
	checkCmd := migrateCommandWithValidationFrontend(t, "migrate", "--check", "--root", root)
	...
	writeCmd := migrateCommandWithValidationFrontend(t, "migrate", "--write", "--check", "--root", root)
