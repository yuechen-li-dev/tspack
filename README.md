...
| Core package | `tspack migrate` | Generate a package.json-based `manifest.migrated.tsx` draft and migration report for human/LLM review; `--package-lock`/default lock detection and default source import scanning add report-only evidence. | Dry-run by default; `--write` writes migration outputs only and never runs scripts. |
