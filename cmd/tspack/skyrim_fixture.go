...
func runSkyrimFixtureCommand(args []string) {
	...
	case "saves":
		...
		runSkyrimSaveList(parseSkyrimSaveListOptions(args[3:]))
	case "fixture":
		switch args[2] {
		case "create":
			runSkyrimFixtureCreate(parseSkyrimFixtureCreateOptions(args[3:]))
		case "inspect":
			runSkyrimFixtureInspect(parseSkyrimFixtureInspectOptions(args[3:]))
		default:
...
func runSkyrimSaveList(opts skyrimSaveListOptions) {
	root, target, profile := loadSkyrimFixtureContext(opts.root, opts.host)
...
func runSkyrimFixtureCreate(opts skyrimFixtureCreateOptions) {
	root, target, profile := loadSkyrimFixtureContext(opts.root, opts.host)
	saveDirectory, source, err := resolveSkyrimSaveDirectory(profile)
	if err != nil {
		failSkyrim("TSPACK_SKYRIM_SAVE_DIRECTORY_UNAVAILABLE", err)
...
func runSkyrimFixtureInspect(fixtureID string, rootInput string, hostInput string, jsonOut bool) {
	_, target, profile := loadSkyrimFixtureContext(rootInput, hostInput)
...
func loadSkyrimFixtureContext(rootInput string, hostInput string) (string, skyrimTargetRef, skyrimHostProfile) {
