...
func printCommandHelp(command string) {
	pages := map[string][]string{
		...
		"update":   {"tspack update", "Resolves declared dependencies and writes ts-lock.toml.", "Use after editing manifest.tsx dependencies or when checking available policy-approved changes.", "Examples:", "  tspack update", "  tspack update react --dry-run", "Behavior:", "  Resolution stays deterministic; resolver preparation fetches package facts in parallel, then commits lockfile truth in a stable serial order.", "  When resolution already fetched an npm tarball, update writes it into the local store immediately and later population skips it.", "  Set TSPACK_RESOLVE_JOBS to tune bounded resolver I/O concurrency and TSPACK_STORE_JOBS to tune cold store population; both default to 24.", "  Dry-run remains read-only and does not create the store, write ts-lock.toml, or materialize node_modules.", "Important flags:", "  --root <path>   Project root", "  --dry-run       Preview lockfile changes", "  --json          Emit machine-readable output", "  --quiet         Reduce output", "Related:", "  sync            Materialize the lockfile", "  outdated        Show available updates", "  check           Validate the result"},
