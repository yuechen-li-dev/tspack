...
func printCommandHelp(command string) {
	pages := map[string][]string{
		...
		"adopt":    {"tspack adopt", "Observes existing npm project metadata without writing files.", "Use during incremental adoption to inspect package.json/package-lock project state before migration.", "Examples:", "  tspack adopt --report", "  tspack adopt --security", "  tspack adopt --security --json", "  tspack adopt --suggest-package packages/ui --root .", "Important flags:", "  --root <path>   Project root", "  --report        Read-only package.json/native adoption summary", "  --security      Read-only observed npm lifecycle/security report with capability warnings and why chains", "  --suggest-package <package-root>   Print a dry-run package.manifest.tsx annotation suggestion", "  --json          Emit machine-readable output, including capabilityWarnings when used with --security", "Behavior:", "  Reads package.json, package-lock.json, and installed package metadata when available.", "  Classifies lifecycle scripts as behavioral capabilities, not CVEs or malware findings.", "  Does not run npm, execute scripts, fetch registry metadata, or write files.", "  --suggest-package reads the package package.json and prints advisory annotatePackage(<PackageAnnotations />) output to stdout.", "Related:", "  npm             Delegate to the real npm CLI", "  migrate         Generate manifest suggestions", "  why             Inspect observed npm reasoning once migrated"},
