...
func printCommandHelp(command string) {
	pages := map[string][]string{
		...
		"update":   {"tspack update", "Resolves declared dependencies and writes ts-lock.toml.", "Use after editing manifest.tsx dependencies or when checking available policy-approved changes.", "Examples:", "  tspack update", "  tspack update react --dry-run", "Behavior:", "  Resolution stays deterministic; cold artifact capture and final store population run separately.", "  When resolution already fetched an npm tarball, update writes it into the local store immediately and later population skips it.", "  Set TSPACK_STORE_JOBS to a positive integer to tune cold store population; the default is 24 workers.", "  Dry-run remains read-only and does not create the store, write ts-lock.toml, or materialize node_modules.", "Important flags:", "  --root <path>   Project root", "  --dry-run       Preview lockfile changes", "  --json          Emit machine-readable output", "  --quiet         Reduce output", "Related:", "  sync            Materialize the lockfile", "  outdated        Show available updates", "  check           Validate the result"},
		"sync":     {"tspack sync", "Materializes dependencies from ts-lock.toml.", "Use after:", "  tspack update", "Examples:", "  tspack sync", "  tspack sync --clean", "Behavior:", "  sync materializes project tool shims in node_modules/.bin for declared tool dependencies.", "  Package files materialize from the local content-addressed store with hardlink-first writes and copy fallback when hardlinks are unavailable.", "  Treat node_modules as immutable generated output; editing dependency files in place can mutate shared store content when hardlinks are used.", "  On fresh machines, sync hydrates missing local store artifacts from the locked source without changing versions or rewriting ts-lock.toml.", "  Cold materialization prints plain [i/n] progress lines on stderr.", "  On Windows, sync retries transient locked-file replacement failures before reporting a diagnostic.", "Important flags:", "  --root <path>   Project root", "  --clean         Rebuild materialized dependency layout", "Related:", "  update          Resolve dependencies into ts-lock.toml", "  check           Validate the current project"},
