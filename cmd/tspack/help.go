...
func printDefaultHelp() {
	...
	fmt.Println("  sync       Materialize locked dependencies")
...
func printCommandHelp(command string) {
	pages := map[string][]string{
		...
		"sync":     {"tspack sync", "Materializes dependencies from ts-lock.toml.", "Use after:", "  tspack update", "Examples:", "  tspack sync", "  tspack sync --clean", "Important flags:", "  --root <path>   Project root", "  --clean         Rebuild materialized dependency layout", "Related:", "  update          Resolve dependencies into ts-lock.toml", "  check           Validate the current project"},
		...
		"run":      {"tspack run", "Runs named RunTargets declared in manifest.tsx.", "Use for dev servers, builds, typechecks, and other project commands owned by the manifest.", "Examples:", "  tspack run dev", "  tspack run build", "  tspack run --list", "Important flags:", "  --root <path>              Project root", "  --package <name>           Select package", "  --env KEY=VALUE            Add environment variables", "  --once                     Stop after readiness", "  --ready-timeout <seconds>  Readiness timeout", "Related:", "  inspect         Inspect a running target", "  check           Validate targets before running"},
