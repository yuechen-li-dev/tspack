...
func printCommandHelp(command string) {
	pages := map[string][]string{
		...
		"run":      {"tspack run", "Runs named RunTargets declared in manifest.tsx.", "Use for dev servers, builds, typechecks, and other project commands owned by the manifest.", "Examples:", "  tspack run dev", "  tspack run build", "  tspack run --list", "Behavior:", "  For runtime node/nodejs targets, run prepends the project tool bin from sync before host PATH.", "Important flags:", "  --root <path>              Project root", "  --package <name>           Select package", "  --env KEY=VALUE            Add environment variables", "  --once                     Stop after readiness", "  --ready-timeout <seconds>  Readiness timeout", "Related:", "  inspect         Inspect a running target", "  check           Validate targets before running"},
