...
```yaml
steps:
  - uses: yuechen-li-dev/tspack/.github/actions/setup-tspack@v0.1.9-m80c.3
    with:
      version: v0.1.9-m80c.3
...
Because the action is stored in this repository, external workflows must use the subdirectory action path. The bounded M80c qualification pins both the action source and binary to `v0.1.9-m80c.3`; `yuechen-li-dev/tspack/setup-tspack@...` would require a future separate `setup-tspack` repository.
