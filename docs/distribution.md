...
Because the action is stored in this repository, external workflows must use the subdirectory action path. The bounded M80b1 qualification pins both the action source and binary to `v0.1.9-m80b1.1`; `yuechen-li-dev/tspack/setup-tspack@...` would require a future separate `setup-tspack` repository.
