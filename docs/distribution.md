...
```yaml
steps:
  - uses: yuechen-li-dev/tspack/.github/actions/setup-tspack@v0.1.9-m80c.4
    with:
      version: v0.1.9-m80c.4

  - run: tspack check --root .
  - run: tspack test --root .
```

The action resolves `latest` through the GitHub Releases API or accepts a pinned tag such as `v0.1.8`, downloads the matching release artifact plus `checksums.txt`, verifies the artifact SHA256, extracts the binary, installs it into a runner-temp bin directory by default, and appends that directory to `GITHUB_PATH`.

Projects may commit `.tspack-version` with one minimum semantic version. The CLI checks it before manifest frontend execution, and the setup action rejects a resolved release below that minimum. Missing files remain backward compatible. Keep the action `uses:` reference new enough to understand the contract.
...
Because the action is stored in this repository, external workflows must use the subdirectory action path. The bounded M80c qualification pins both the action source and binary to `v0.1.9-m80c.4`; `yuechen-li-dev/tspack/setup-tspack@...` would require a future separate `setup-tspack` repository.
...
```sh
./scripts/package-release.sh --goos linux --goarch amd64 --version v0.1.8
```
