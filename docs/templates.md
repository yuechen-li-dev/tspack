...
A practical MachinaLayout example lives at `internal/templates/testdata/local-concepts/machina-react-app`: it composes built-in React/Vite/TypeScript concepts with local `my-company.machina-layout`, contributes the real npm `machinalayout` dependency, adds a small `src/machina-layout.tsx` integration file plus `src/style.css`, and renders `manifest.tsx` from the explicit concept stack. This remains local-template/local-concept behavior, not a built-in MachinaLayout public concept, remote marketplace entry, script hook, or template inheritance mechanism. Concepts can add supported dependencies, tools, peers, run targets, and safe files, but they still cannot patch arbitrary files.
