...
A check-clean React/Vite local template should list the built-in React, browser SPA, Vite, TypeScript, workspace, manifest-boundary, update-policy, and security-policy concepts, add any local concept dependency/run-target/file contributions, and project the ordinary Vite app files. The test fixture at `internal/templates/testdata/local-concepts/concept-manifest-app` is the minimal example: it uses a local design-system concept, renders `manifest.tsx` from concepts, contributes `src/design-system.ts`, and still projects `src/main.tsx` plus Vite/TypeScript config files. A practical Tailwind example lives at `internal/templates/testdata/local-concepts/tailwind-react-app`: it composes built-in React/Vite/TypeScript concepts with local `my-company.tailwind`, contributes `tailwindcss` as an app dependency, `@tailwindcss/vite` as a tool, plus `src/style.css`, and projects Vite config normally rather than patching an existing file.

A practical MachinaLayout example lives at `internal/templates/testdata/local-concepts/machina-react-app`: it composes built-in React/Vite/TypeScript concepts with local `my-company.machina-layout`, contributes the real npm `machinalayout` dependency, adds a small `src/machina-layout.tsx` integration file plus `src/style.css`, and renders `manifest.tsx` from the explicit concept stack.

A composed Tailwind + MachinaLayout example lives at `internal/templates/testdata/local-concepts/tailwind-machina-react-app`: it lists `my-company.tailwind` and `my-company.machina-layout` explicitly after the React/browser/Vite/TypeScript substrate and before the TSPack policy concepts. The base fixture owns `src/main.tsx`, `src/App.tsx`, `vite.config.ts`, `index.html`, and package/config files; the Tailwind concept owns `src/style.css`; and the MachinaLayout concept owns `src/machina-layout.tsx`. Dependency and tool contributions merge into the concept-generated manifest, while file ownership remains deterministic because neither concept patches arbitrary files or silently edits the app shell.

These examples remain local-template/local-concept behavior, not built-in Tailwind or MachinaLayout public concepts, remote marketplace entries, script hooks, or template inheritance mechanisms. Concepts can add supported dependencies, tools, peers, run targets, and safe files, but they still cannot patch arbitrary files.

If the `[generation]` table is absent, existing behavior remains: manifest-like local concept contributions fail with `TSPACK_TEMPLATE_CONCEPT_UNSUPPORTED_CONTRIBUTION` instead of being silently dropped. Unknown generation manifest modes fail with `TSPACK_TEMPLATE_INVALID`. Unsupported concept manifest fields in concept-rendered mode also fail loudly. Concepts can add supported dependencies, tools, peers, run targets, and safe files, but they cannot patch arbitrary files; config changes must be represented as explicit projected files or future structured projections. Env rendering, service requirements, pack metadata, remote concepts, JavaScript/TypeScript concept files, scripts, package installation during init, arbitrary text patches, template inheritance, and public concept registries are not supported.
