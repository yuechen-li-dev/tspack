...
`nodejs`, `bun`, and `deno` are the supported profile names; omitted runtime defaults to `nodejs`. The `nodejs` profile is the behavior-preserving compatibility baseline for existing projects: `<Workspace name="demo">` and `<Workspace name="demo" runtime="nodejs">` are equivalent after normalization and should preserve check, update, sync, pack, why, run, test, format/lint, inspect, and doctor behavior. The product contract is to change the runtime profile while keeping the project contract stable: the M42e runtime-switch fixtures show `nodejs`, `bun`, and `deno` variants whose project contract, dependency graph, pack metadata, and why explanations remain stable while only the selected runtime profile changes. Runtime profile selection does not mean package-manager delegation: TSPack still owns dependency resolution, lockfiles, materialization, checks, packaging, package-manager truth, and lifecycle security policy. Runtime profile selection is not npm package metadata and must not leak into generated pack `package.json` output. The runtime profile seam now has constrained Bun and Deno proofs: `<Workspace runtime="bun">` and `<Workspace runtime="deno">` report cleanly in doctor, and explicit `RunTarget.runtime: "bun"` or `RunTarget.runtime: "deno"` launches the selected runtime with the declared argv payload. It still is not package-manager or task-runner switching, `bun install`, `bun add`, `deno task`, `deno install`, `deno cache`, runtime-owned lockfile support, runtime-owned materialization, or automatic RunTarget inheritance.
