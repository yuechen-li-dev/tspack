...
Unsupported manifest contributions from local concepts now fail loudly for local templates that do not have concept manifest rendering, so dependency/tool/run-target intent cannot silently disappear. M60g adds opt-in generic concept-rendered manifest support for local templates through `[generation] manifest = "concept"`, allowing local TOML concepts to contribute supported manifest rows deterministically. Future work may add richer projections, Tailwind/MachinaLayout/Shadcn/Storybook-style built-ins, concept-authored tsconfig/vite config, and much later remote registry exploration. M60f does not add remote concepts, script execution, package installation during init, template inheritance, a public marketplace, or package-manager behavior changes.
