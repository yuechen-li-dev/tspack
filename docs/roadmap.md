...
- Future: add a MachinaLayout local concept, compose Tailwind + MachinaLayout, add richer concept-authored projections/config, add env/service/pack support for backend concepts, and later explore `docs.storybook`, `ui.shadcn`, `node.service`, and NestJS-oriented service concepts after the core model converges.
