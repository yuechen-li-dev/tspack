...
- When no paths are supplied, `tspack format` and `tspack lint` derive the same source-scoped path set as `tspack check --format`; materialized stores and generated output are never passed to Biome.
...
TSPack's default Biome config is intentionally explicit. The same default is used by `tspack format`, the temporary config for `tspack check --format`, and the `biome.json` generated by `tspack init`:
...
- import organization enabled through Biome 2's `assist.actions.source.organizeImports`;
- generated/project outputs excluded through both Biome 2 scanner ignores and negated `files.includes` entries;
...
`tspack check --format` runs normal project validation plus a read-only Biome format check scoped to source/project paths relative to the selected root. It uses the same backend resolution and config behavior as `tspack format --check`, including the temporary default config and stderr status line when no project `biome.json` or `biome.jsonc` exists. It does not write files and does not pass Biome `--write` or Biome `--check`. Run `tspack format` to apply formatting changes.

With `tspack check --format --json`, Biome stdout/stderr is captured instead of streamed so stdout remains parseable JSON. Format failures are included in the normal check JSON diagnostics as `TSPACK_FORMAT_CHECK_FAILED`; captured formatter output is sanitized so JSON details do not contain raw ANSI escape sequences.
...
The default ignore list excludes generated and artifact directories: `.tspack/**`, `node_modules/**`, `dist/**`, `tspack-artifacts/**`, `coverage/**`, `.git/**`, `build/**`, `.turbo/**`, and `.vite/**`.
