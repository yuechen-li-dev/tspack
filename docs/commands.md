...
| `tspack build [target]` | Topologically build declared compiler targets through tsc, Copeland, or bounded ScriptC adapters; compiler descriptors preserve tool/config/source/package/artifact provenance. | No / **Yes (build artifacts and metadata)** | Does not mix compiler source semantics, install tools inside adapters, or load ScriptC archives directly into Node. | `docs/dev/m71-compiler-target-boundary.md`, `docs/dev/m71a-scriptc-hotpath.md` |
