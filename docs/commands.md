...
| `tspack migrate` | Convert package.json metadata into a reviewable `manifest.migrated.tsx` draft and `tspack-migration.md` report, including script classification and report-only RunTarget suggestions. Dry-run by default; `--write` creates files. | **Yes with `--write` (migration outputs only)** / No | Does not overwrite `manifest.tsx`, mutate package.json, translate lockfiles to TSPack locks, mutate source imports, install, execute scripts, migrate npm scripts into active RunTargets, or run update/sync/check. | `docs/migrate.md` |
