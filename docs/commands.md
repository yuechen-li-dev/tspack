...
| `tspack test` | Run test backends (native xTest and/or Vitest); supports `--compact` for native xTest compact text output and `--xtest-bridge <path>` for explicit native bridge resolution. | No / No | Not a generic task runner. | `docs/test-command.md`, `docs/native-test-harness.md` |
