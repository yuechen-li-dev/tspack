...
| `tspack run [target]` | Start declared manifest `RunTargets`, list them with `--list [--json]`, and scope selection with `--package <name>`; status stays on stderr. | No / No | Not `npm run`; no package.json script inference; declared cwd controls workspace-root vs package-root command resolution. | `docs/run.md` |
